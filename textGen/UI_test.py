...
def TTS(word, file_path):
    """模擬生成語音檔案"""
    print(f"生成語音: {word}")
    # 模擬生成語音檔案
    with open(file_path, "w") as f:
        f.write("Simulated audio content")  # 實際應該調用語音生成庫
    print(f"語音檔案已儲存到: {file_path}")
    return file_path

def generate_audio(input_audio_path, output_path="textGen/UI/change_part"):
    global original_list
    # 遍歷原始列表，生成有變化的語音
